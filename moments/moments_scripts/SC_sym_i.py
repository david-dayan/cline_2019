...
def demo_model(params, ns):
    ...
    """
    SC_sym_i: Split with no gene flow, followed by period of symmetrical gene flow, with reduced selection at P sites.
    ...
    P: neutral portion
    mi: migration at selected sites
    """
    nu1, nu2, m, mi, T1, T2, P , p_misid= params
    ...
    sym_mig = numpy.array([[0,mi], [mi,0]])
    nomig = numpy.array([[0,0], [0,0]])
    fsi.integrate([nu1,nu2], T1, m=nomig, dt_fac=0.01)
    fsi.integrate([nu1,nu2], T2, m=sym_mig, dt_fac=0.01)
...
fs = moments.Spectrum.from_file("/home/ddayan/fundulus/moments_data/SFS/fs_40_40.fs")
...
upper=[20,20,20,20,20,20,1,1]
lower=[ 0.001,0.001,0.001,0.001,0.001,0.001,0.001,0.0001]
